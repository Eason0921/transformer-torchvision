...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # Pass through the pre-attention projection: b，lq，(n*dv)
        # Separate different heads: b，lq ，n，dv

        # q k v 分别经过一个线性层再改变维度
        # 由(sz_b, len_q, n_head*d_k) => (sz_b, len_q, n_head, d_k)
        # (sz_b, len_q, 8*64) => (sz_b, len_q, 8, 64)
        ...
        # Transpose for attention dot product: b ，n，lq，dv
        # 交换维度做attention
        # 由(sz_b, len_q, n_head, d_k) => (sz_b, n_head, len_q, d_k)
        # (sz_b, len_q, 8, 64) => (sz_b, 8, len_q, 64)
        ...
        #q_shape=(sz_b, 8, len_q, 64)
        # Transpose to move the head dimension back: (b ,lq,n,dv)
        # Combine the last two dimensions to concatenate all the heads together: b x lq x (n*dv)
        #连接所有的注意力头
        # qshape：(sz_b, 8, len_k, 64) => (sz_b, len_k, 8, 64) => (sz_b, len_k, 512)
...
# 其实就是一个MLP而已
# MLP的实现也仅是对空间位置的 feature 进行全局感知，没有进行 channel 之间的交互。
